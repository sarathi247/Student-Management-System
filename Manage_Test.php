...
<body >
    ...
    <div class="manage">
        ...
        <!-- Search Form -->
        <form method="GET">
            <input type="text" name="search" class="search-bar" placeholder="Search by Batch ID" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            ...
            $searchQuery = " WHERE batchid LIKE '%$search%'";
        }

        // Fetching records from the database
        $sql = "SELECT * FROM test" . $searchQuery;
        $result = $conn->query($sql);

        echo "<br/><div class='back'><a href='Add_test.php'>Add Test</a></div><br/>";

        if ($result->num_rows > 0) {
            echo "<table border='1' style='width:100%; text-align:center; font-size:18px; border-collapse: collapse;'>
                    <tr>
                        <th>No</th><th>Batch ID</th><th>Test No</th><th>Topic</th><th>Date</th><th>Action</th>
                    </tr>";
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['date']}</td>
                        <td>
                            <a class='edit' href='update.php?no={$row['no']}'>Edit</a> |
                            <a class='delete' href='mark_delete.php?no={$row['no']}'>Delete</a>
                        </td>
                    </tr>";
            }
            echo "</table>";
