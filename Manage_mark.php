...
<body >
    ...
    <div class="manage">
        ...
        <!-- Search Form -->
        <form method="GET">
            <input type="text" name="search" class="search-bar" placeholder="Search by Student ID" value="<?php echo isset($_GET['search']) ? $_GET['search'] : ''; ?>">
        ...
        if (isset($_GET['search']) && !empty($_GET['search'])) {
            ...
            $searchQuery = " WHERE studentid LIKE '%$search%'";
        }

        // Fetching records from the database
        $sql = "SELECT * FROM mark" . $searchQuery;
        $result = $conn->query($sql);

        echo "<br/><div class='back'><a href='Add_Mark.php'>Add Mark</a></div><br/>";

        if ($result->num_rows > 0) {
            echo "<table border='1' style='width:100%; text-align:center; font-size:18px; border-collapse: collapse;'>
                    <tr>
                        <th>No</th><th>Test No</th><th>Student ID</th><th>Mark</th><th>Action</th>
                    </tr>";
            while ($row = $result->fetch_assoc()) {
                echo "<tr>
                        ...
                        <td>{$row['mark']}</td>
                        <td>
                            <a class='edit' href='mark_update.php?no={$row['no']}'>Edit</a> |
