...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT * FROM test WHERE no='$no'";
    $checkResult = $conn->query($checkQuery);

    if ($checkResult->num_rows > 0) {
        // Redirect to a new page with existing record details
        header("Location: existing_test.php?no=$no");
        exit();
    } else {
        // Insert new data if no duplicate found
        $sql = "INSERT INTO test (no, batchid, testno, topic, date) 
                VALUES ('$no', '$batchid', '$testno', '$topic', '$date')";

        if ($conn->query($sql) === TRUE) {
            header("Location: Manage_Test.php?msg=New record created successfully");
            exit();
        } else {
...
<body >
    ...
    <div class="section">
        <div class="content">
            <p class="main">Add Test </p>
            <hr>
            <p style="margin-left: 30px;font-size: 15px;">Home / Test/ Add Test</p>
        ...
        <div class="content1">
            ...
            <form action="" method="post" >
                <p>No</p>
                <input type="number" name="no" maxlength="5" oninput="validateNumber(this)" required>
                <p>Batch Id</p>
                <input type="number" name="batchid" maxlength="5" oninput="validateNumber(this)" required>
                <p>Test No</p>
                ...
                <p></p>
                <button type="submit" name="sumbit">Save</button><a href="Manage_Test.php"></a>
            </form>
        </div>
